...
static float gaussian3[3*3] = {
	1.0 / 16.0,	2.0 / 16.0,	1.0 / 16.0,
	2.0 / 16.0,	4.0 / 16.0,	2.0 / 16.0,
	1.0 / 16.0,	2.0 / 16.0,	1.0 / 16.0	
};

static float gaussian5[5*5] = {
...
void generateFilter(float *fP, int w) {
  float *precomputed = gaussian5;
  for (int r = -w; r <= w; r++)
  {
...
int main(int argc, char *argv[]) {
    ...
    //generate filter
    float *filter = new float[filterSize * filterSize * 4];
    generateFilter(filter, halfFilterSize);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err,krnl_convolute =cl::Kernel(program, "convolute", &err));
    ...
    OCL_CHECK(err,cl::Buffer buffer_filter(context,CL_MEM_USE_HOST_PTR,filterSize * filterSize * 4 * sizeof(float),filter,&err));
    ...
    krnl_convolute.setArg(0, width);
    krnl_convolute.setArg(1, halfFilterSize);
    krnl_convolute.setArg(2, halfFilterSize);
    krnl_convolute.setArg(3, buffer_inImage);
    krnl_convolute.setArg(4, buffer_outImage);
    krnl_convolute.setArg(5, buffer_filter);
    OCL_CHECK(err, err = q.enqueueNDRangeKernel(krnl_convolute, NULL, gSize, wgSize, NULL, NULL));
...
delete []filter;
// 释放内核对象
if (krnl_convolute() != NULL) {
    clReleaseKernel(krnl_convolute.get());
}
